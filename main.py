...
import os
import pickle
# from keras.utils import ImageDataGenerator, array_to_img, img_to_array, load_img
import time
import cv2
import numpy as np
...
log_pickle_model = pickle.load (open("mainmodel.pkl", 'rb'))
sift = cv2.xfeatures2d.SIFT_create ()

size = 128
intensity_model = load_model('squeezenet.h5', compile=False)
...
def prediction (file):
    ...
    if log_pickle_model.predict (images_sift_glcm_test) == 1:
        ...
        circles = cv2.HoughCircles (mask, cv2.HOUGH_GRADIENT, 1.5, 100000, param1=80, param2=40, minRadius=0,
                                    maxRadius=0)
        x, y, r = 0, 0, 0
